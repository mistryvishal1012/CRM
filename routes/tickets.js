...
router.get("/", isLoggedIn, async function(req, res){
	...
	res.render("tickets/tickets", {tickets: tickets,title : "tickets"});
});	
...
router.get("/add", isLoggedIn, async function(req, res){
	...
	res.render("tickets/add_ticket", {users: users,title : "tickets"});
})
...
router.get("/:id", isLoggedIn, async function(req, res){
	...
	try {
		if(!ObjectId.isValid(req.params.id)){
			req.flash("error","Ticket Does not Exits");
	...
	res.render("tickets/ticket", {ticket: ticket,title : "tickets"});
});
...
router.get("/:id/edit", isLoggedIn, isManager, async function(req, res){
	...
	res.render("tickets/edit_ticket", {ticket: ticket, users: users, due_date: due_date,title : "tickets"}); 
});
...
router.put("/:id", isLoggedIn, isManager, async function(req, res){ 
	...
	try {
		...
		ticketToUpdate = await Ticket.findById(req.params.id);
		if(!ticketToUpdate){
			req.flash("error","Ticket Does not Exits");
			return res.redirect("/tickets")
		}
...
router.get("/:id/complete_form", isLoggedIn, async function(req, res){
	...
	res.render("tickets/complete_ticket", {ticket: ticket, users: users,title : "tickets"});
});
...
// 	Update Ticket
	...
	try {
		...
		completed_ticket = await Ticket.findByIdAndUpdate(req.params.id,{
			'complete' : true
		})
